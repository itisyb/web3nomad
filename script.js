...
element.addEventListener('change', (event) => {
  if (current_value < event.target.value) {
    ...
    var main_element = document.querySelector('#jobs-wrapper');
    var parent = document.createElement('div');
    parent.setAttribute('class', 'parent-job-wrapper');
...
<div fs-selectcustom-element="dropdown" data-delay="0" data-hover="false"
    ...
    var underlay_comp = document.createElement('div');
    underlay_comp.setAttribute('class', 'underlay');
    margin_job_details_comp.append(underlay_comp)
    ...
    name="Min-sal-${current_value}" data-name="Min-sal-${current_value}" placeholder="Minimum Salary"
    id="Min-sal-${current_value}" required=""><input type="number"
    class="fs-input_field w-node-d997b593-e85c-2e53-aa83-df1df3336353-8efa05fd w-input" maxlength="256"
    name="Max-sal-${current_value}" data-name="Max-sal-${current_value}" placeholder="Maximum Salary"
    id="Max-sal-${current_value}" required="">
<p id="w-node-_55a05724-e6fe-45f8-1a41-81c1b9961bc6-8efa05fd" class="text-weight-light">No worries if the company is not
    ...
    name="Min-eq-${current_value}" data-name="Min-eq-${current_value}" placeholder="Minimum Equity (If provided)"
    id="Min-eq-${current_value}"><input type="text"
    class="fs-input_field w-node-_16fff19b-86dc-5ba6-ccd7-01d1f071dd22-8efa05fd w-input" maxlength="256"
    name="Max-eq-${current_value}" data-name="Max-eq-${current_value}" placeholder="Maximum Equity (If provided)"
    id="Max-eq-${current_value}">
<div id="w-node-c02f9cb0-2c77-6a1b-90fb-f7640725f3a4-8efa05fd" class="remote--switech"><label
        ...
        <div class="fs-checkbox_toggle-dot-7"></div><input type="checkbox" class="w-checkbox-input fs-checkbox_button-7"
            name="option-pay-crypto-${current_value}" fs-cmsfilter-field="featured"
            data-name="option-pay-crypto-${current_value}" aria-label="add_description_here"
            id="option-pay-crypto-${current_value}"><span class="fs-checkbox_label-7 w-form-label"
            for="option-pay-crypto-${current_value}">Checkbox 8</span>
    ...
    main_element.append(parent) 
    console.log('greater')
    current_value = event.target.value;
  } else if(current_value > event.target.value){
    console.log('smaller');
    var last = document.getElementsByClassName('parent-job-wrapper');
